    ...
    public void testGeneratingForbiddenRectangles() {
        ...
        for (Cell[] row : roomGenerator.cells) {
            for (Cell cell : row) {
                ...
                // Hack to get around fact that you cannot
                // reassign variables values in a lambda.
                final boolean[] containsForbidden = {false};
                roomGenerator.getForbiddenRectangles(cellLength).forEach(
                    (point2D, rectangles) ->  {
